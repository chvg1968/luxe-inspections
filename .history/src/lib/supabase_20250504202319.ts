...
export const loadSequentialPhotos = async (
  ...
  try {
    ...
    // Normalizar el nombre de la villa para usarlo como carpeta
    const normalizedVillaName = 'villapalacio'; // Por ahora usamos este valor fijo
    
    // Intentar listar todas las fotos disponibles en el bucket
    console.log(`Listando fotos en la carpeta: ${normalizedVillaName}`);
    ...
    if (error) {
      ...
      console.warn('Intentando listar archivos sin especificar carpeta');
      ...
      if (rootResult.error || !rootResult.data || rootResult.data.length === 0) {
        console.error('No se pudieron listar archivos en ninguna carpeta');
        return [];
      }
    ...
    const imageFiles = files.filter(file => 
      /\.(jpe?g|png|jpg|gif|webp|tiff?)$/i.test(file.name)
    );
    
    console.log(`De los cuales ${imageFiles.length} son imágenes`);
    
    if (imageFiles.length === 0) {
      console.warn('No se encontraron imágenes en el bucket');
      return [];
    }
    ...
    for (const file of imageFiles) {
      ...
      const { data } = supabase.storage
        .from('photos')
        .getPublicUrl(`${normalizedVillaName}/${file.name}`);
    ...
    console.log(`Cargadas ${photos.length} fotos directamente desde el bucket`);
    return photos;
  } catch (error) {
